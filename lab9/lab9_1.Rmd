...
```{r}
relig_income %>% 
  pivot_longer(-religion, #dont move
               names_to="income", #give name to new column
               values_to="number") #move the count into a new column
```
...
billboard %>% 
  pivot_longer(
    cols=starts_with("wk"),
    names_to = "week",
    names_prefix = "wk", #gets rid of the prefix wk
    values_to = "rank",
    values_drop_na = T
  )
```
...
plant_data %>% 
  pivot_longer(
    ...
    names_to = "day",
    names_prefix = "day", #gets rid of the prefix wk
...
qpcr_tidy %>% 
  ...
  separate(replicate, into=c("reo", "r_number"), sep=3) %>% 
