...
This data is tidy. Eeach variable has its own column, each observation its own row, and each value its own cell.
...
sydneybeaches_long %>% 
  separate(Date, into=c("day", "month", "year"), sep="/") %>% 
  group_by(Site, year) %>% 
  summarize(entero_year_avg = mean(`Enterococci (cfu/100ml)`, na.rm=T))
```
...
sydneybeaches_long %>% 
  ...
  filter(year =="2013") %>% 
  group_by(Site) %>% 
  summarize(entero_2013_avg = sum(`Enterococci (cfu/100ml)`, na.rm=T)) %>% 
  arrange(desc(entero_2013_avg))
```
