...
ggplot(data=homerange,
  ...
  labs(title="Animals Group Counts")
```
...
Hipoliti have the smallest mean homerange size.
...
Tarandus have the largest mean homerange size.

**7. Which carnivorous mammals have the largest homeranges?**
```{r}
homerange %>% 
  select(common.name, taxon, trophic.guild, mean.hra.m2) %>% 
...
```{r}
homerange %>% 
  ggplot(mapping=aes(x=mean.mass.g, y=mean.hra.m2))+
  scale_x_log10()+
  scale_y_log10()+
  geom_point()+
  geom_smooth(method=lm)+
  labs(title="Mean Body Mass (g) vs Mean Homerange Size (m2)",
       x="Mean Body Mass (g)",
       y="Mean Homerange Size (m2)")
```
...
homerange %>% 
  ...
  labs(title="Homerange Size Comparison",
       x="Taxon",
       y="Mean Homerange (m2)")
```
...
homerange %>% 
  ...
  filter(taxon=="marine fishes" | taxon=="lake fishes") %>% 
  arrange(desc(mean.hra.m2)) %>% 
  ggplot(mapping=aes(x=taxon, y=mean.hra.m2))+
  scale_y_log10()+
  geom_boxplot(mapping=aes(fill=taxon))+
...
Do lake or marine fishes have a larger mean homerange?
